...
namespace SICSA.clases
{
    ...
    {
        public static void nuevo_acuerdo(string accion, int cod_punto, DateTime fecha_registro, string acuerdo_largo, string acuerdo_corto, DateTime fecha_ini, int periodo, int usuario, int cd_acuerdo, int cd_tipo_acuerdo)
        {
        ...
        public static List<datos_acuerdo> datos_acuerdo(int cd_acuerdo)
        {
            ...
            using (SqlConnection _conn = conexion.conectarBD())
            {
                ...
                strquery = "SELECT DS_ACUERDO_COMP, DS_ACUERDO_CORTO, CD_TIPO_ACUERDO, FE_FECHA_INI, PERIODO_DIAS  FROM ACU_ENCA_ACUERDOS   WHERE CD_ENCA_ACUERDOS = " + cd_acuerdo;
                SqlCommand _cmd = new SqlCommand(strquery, _conn);
                SqlDataReader _reader = _cmd.ExecuteReader();
                while (_reader.Read())
                {
